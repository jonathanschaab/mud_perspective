...
* **Anaphora Resolution (Smart Pronouns):** The engine tracks the most recently mentioned entity in the current `RenderContext`. If you use a pronoun tag (like `{target:subj}`) for an entity that isn't the current subject of the narrative, the engine automatically expands it to their full name with a definite article (e.g., `The goblin`) to prevent ambiguous pronouns. On subsequent uses, it correctly outputs the pronoun (e.g., `It`). Possessive pronouns (like `{target:poss}`) intelligently fall back to possessive nouns (e.g., `The goblin's`). You can clear this memory manually across paragraphs via `ctx.clear_anaphora()`.
